#ifndef __VINS_TO_MAVROS
#define __VINS_TO_MAVROS
...
void TF_EDN_to_ENU::tf_world_to_map()
{
    ...
    tf2::Quaternion q;
    q.setRPY(0, 0, M_PI/2); // 绕 source_frame 定轴转动
...
void TF_EDN_to_ENU::vision_callback(nav_msgs::OdometryConstPtr pMsg)
{
    ...
    {
        /* 标准坐标变换写法 */
        // // 1. 从 body 坐标系 到 base_link 坐标系 的坐标变换 (在 world 坐标系下)
        // p_mav = Eigen::Vector3d(pMsg->pose.pose.position.x, pMsg->pose.pose.position.y, pMsg->pose.pose.position.z);
        // q_mav = Eigen::Quaterniond(pMsg->pose.pose.orientation.w, pMsg->pose.pose.orientation.x, pMsg->pose.pose.orientation.y, pMsg->pose.pose.orientation.z);
        ...
        // // 2. 将 base_link 坐标系 从 world 坐标系转化到 map 坐标系
        // roll = Eigen::AngleAxisd(0, Eigen::Vector3d::UnitX());
        // pitch = Eigen::AngleAxisd(0, Eigen::Vector3d::UnitY());
        // yaw = Eigen::AngleAxisd(-M_PI/2, Eigen::Vector3d::UnitZ());
        // _q_mav = roll * pitch * yaw;

        // p_mav = _q_mav * p_mav;
        // q_mav = q_mav * _q_mav;

        /* 精简坐标变换写法 */
        p_mav = Eigen::Vector3d(pMsg->pose.pose.position.y, 
                                -pMsg->pose.pose.position.x, 
                                pMsg->pose.pose.position.z);

        v_mav = Eigen::Vector3d(pMsg->twist.twist.linear.y, 
...
void TF_EDN_to_ENU::command_callback(quadrotor_msgs::PositionCommandConstPtr pMsg)
{
    ...
    cmd.position.x = pMsg->position.y;
    cmd.position.y = -pMsg->position.x;
    cmd.position.z = pMsg->position.z;

    cmd.velocity.x = pMsg->velocity.y;
    cmd.velocity.y = -pMsg->velocity.x;
    cmd.velocity.z = pMsg->velocity.z;

    cmd.acceleration.x = pMsg->acceleration.y;
...
/* 使用 tf 搜索从 base_link 坐标系到 map 坐标系的坐标变换 */
// 本方法可以获取 base_link 在 map 坐标系下的位姿，但是频率达不到要求，因此弃用
// void TF_Broadcaster_D435i::tf_map_to_baselink()
// {
//     std::string source_frame_id = "map";
//     std::string target_frame_id = "base_link";

//     tf2_ros::TransformBroadcaster dy_broadcaster_;
    
//     tf2_ros::Buffer buffer;
//     tf2_ros::TransformListener listener(buffer);

//     ROS_INFO("try map to base_link");
//     try
//     {
//         // ros::Duration(1).sleep();
//         ts_ = buffer.lookupTransform("map", "fcu", ros::Time(0), ros::Duration(1));
...
//         dy_broadcaster_.sendTransform(ts_);
//     }
...
{
    quadrotor_msgs::PositionCommand cmd = *pMsg;
